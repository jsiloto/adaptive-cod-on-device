...
public class PrePostProcessor {
    ...
    private static int mOutputRow = 76725; // as decided by the YOLOv5 model for input image of size 640*640
    private static int mOutputColumn = 95; // left, top, right, bottom, score and 80 class probability
    ...
    static ArrayList<Result> outputsToNMSPredictions(float[] outputs, float imgScaleX, float imgScaleY, float ivScaleX, float ivScaleY, float startX, float startY) {
        ...
        for (int i = 0; i< mOutputRow; i++) {
            float score = outputs[i* mOutputColumn +4];
            ...
            if (score > mThreshold) {
                ...
                Result result = new Result(cls, outputs[i*95+4], rect);
                results.add(result);
            }
        }
