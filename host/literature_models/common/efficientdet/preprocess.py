...
def aspectaware_resize_padding2(image, width: int, height: int):
    num, c, old_h, old_w  = image.shape
    assert num == 1
    if old_w > old_h:
        new_w = width
        new_h = int(width / old_w * old_h)
...
def preprocess(ori_imgs, max_size=512, mean=(0.485, 0.456, 0.406), std=(0.229, 0.224, 0.225)):
    ...
    ori_imgs = [img.permute(1, 2, 0).numpy() for img in ori_imgs] # Put channels last
    ori_imgs = [255*img[..., ::-1] for img in ori_imgs] # invert colors

    normalized_imgs = [(img[..., ::-1] / 255 - mean) / std for img in ori_imgs] # normalize
